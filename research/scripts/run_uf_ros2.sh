...
#
# Usage:
#   bash research/scripts/run_uf_ros2.sh [path/to/rosbag]
#
...
BAG="${1:-${REPO_ROOT}/research/downloads/data/occlusion01.bag}"
...
if [ ! -f "${BAG}" ]; then
    ...
    echo "into research/downloads/data/ then re-run." >&2
    exit 1
fi
...
echo "  2) uf_node /opt/ultrafusion/config/m3dgr/uf_m3dgr_ros2_lvwio.yaml --ros-args -p use_sim_time:=true"
echo "  3) ros2 bag play /data/$(basename "${BAG}") --clock"
